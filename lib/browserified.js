(function () {
...
})()(
  {
    ...
    2: [
      function (require, module, exports) {
        ...
        addChatMessage = function (nick, message, data) {
          ...
          if (data) {
                ...
                .forEach(function (badge) {
                  ...
                  var targets = Object.keys(badgeList || {})
                    .filter(function (name) {
                      return name.indexOf(badgeName + "/") == 0;
                    })
                    .filter(function (name) {
                      if (!isNaN(badgeTier)) {
                        return Number(name.split("/")[1]) <= badgeTier;
                      }
            ...
            if (chatBadgeBox.children.length == 0) {
          ...
          if (
            chatMessageBox.innerHTML.replace(/(<[^>]*>)|\s/g, "").length == 0
          ) {
          ...
          if (configData.msgExistDuration > 0) {
            setTimeout(function () {
              applyMessageRemove(chatOuterBox);
        ...
        var concatChatMessage = function (nick, message, data) {
          ...
          if (
            lChild &&
            lChild.getElementsByClassName("chat_nickname_box")[0].innerHTML ==
              nick
          ) {
            if (typeof applyMessage != "undefined") {
        ...
        var banChatMessage = function (nick) {
          ...
          if (children && children.length > 0) {
            for (var index in children) {
              ...
              children[index].lower.msg.innerHTML = "&lt;message deleted&gt;";
            }
          }
        };
        ...
        var checkComplete = function () {
          ...
          if (++completeCount == num) {
            var width = Number(
              getComputedStyle(document.body).width.slice(0, -2)
            );
            ...
            var chat = addChatMessage(
              ...
              '<center style="line-height:1em; box-sizing:border-box;">' +
                ...
                "font-size:" +
                width +
                "; line-height:1em; color:white !important;" +
        ...
        debugLog = function (dat) {};
        {
          ...
          if (configDataLength == 0) {
            configLoadMessage =
              "설정 파일(lib/config.js)을 로드하는 데 문제가 생겨 기본 설정을 사용합니다.<br />";
            Object.assign(configData, configDefault);
          } else {
            var unloadedConfigs = Object.keys(configDefault).reduce(function (
              acc,
              cur
            ) {
              if (configData[cur] === undefined) {
                configData[cur] = JSON.parse(
                  JSON.stringify(configDefault[cur])
                );
                return ++acc;
              }
              return acc;
          ...
          document.head.appendChild(
            (function () {
              if (configData.themeURI == "") {
                ...
                configData.themeURI[configData.themeURI.length - 1] != "/"
              ) {
                configData.themeURI += "/";
              }
          ...
          globalBadgeRequest.onreadystatechange = function (evt) {
            if (globalBadgeRequest.readyState == 4) {
              if (globalBadgeRequest.status == 200) {
                ...
                if (
                  configData.badgeChannelId &&
                  Number(configData.badgeChannelId) > 0
                ) {
                  var channelBadgeUrl =
        ...
        var cheerRegExp = new RegExp(
          "(\\s|^)(" + cheerList.join("|") + ")(\\d+)(\\s|$)",
          "i"
        );
        ...
        if (configData.loadTwitchCons) {
          ...
          debugLog(
            "설정에 따라 트위치 이모티콘과 구독콘을 불러오지 않았습니다."
          );
          checkComplete();
        }
        ...
        if (configData.loadDcCons) {
          loadDcCon = function () {
            ...
            if (configData.dcConsURI == "" || configData.dcConsURI == "./") {
              ...
              configData.dcConsURI[configData.dcConsURI.length - 1] != "/"
            ) {
              configData.dcConsURI += "/";
            }
          ...
          switch (exe) {
            ...
            case "scale":
              if (arg == "") {
                debugLog(
                  "현재 배율 : " +
                    (window.localStorage.getItem("scale") || 100) +
                    "%"
                );
                return true;
              }
              ...
              if (line[0] == ":") {
                ...
                switch (args[1]) {
                  ...
                  case "JOIN": // 서버에 유저가 접속
                    if (args[0].search("justinfan") != -1) {
                      debugLog(args[2].substring(1) + "에 접속했습니다.");
                    }
                ...
                switch (args[1]) {
                  ...
                  case "USERNOTICE":
                    switch (twitchArgs["msg-id"]) {
                      ...
                      case "resub": // 구독 메세지 수신
                        data.subMonths = Number(
                          twitchArgs["msg-param-cumulative-months"]
                        );
                        break;
                  ...
                  case "PRIVMSG": // 채팅 수신
                    ...
                    var realNick = "";
                    if (
                      configData.useDisplayName &&
                      displayNick.replace(/\s/g, "") != ""
                    ) {
                      ...
                      if (data.badges.toString().search("broadcaster") != -1) {
                        ...
                        for (var index in configData.commands) {
                          ...
                          if (message.search(cmd.msg) == 0) {
                            ...
                            isBreak =
                              isBreak || commandExecute(cmdText, cmdArgument);
                          }
                        }
                    ...
                    message.split(/\s/).forEach(function (phrase) {
                      if (phrase.match(linkRegExp)) {
                        if (
                          ...
                          (configData.linkReplaceMsg || "").length > 0
                        ) {
                          message = message.replace(
                            phrase,
                            configData.linkReplaceMsg
                          );
                        }
                      }
                    });
                    ...
                    if (!data.color || data.color == "") {
                      var n =
                        realNick.charCodeAt(0) +
                        realNick.charCodeAt(1) * new Date().getDate();
                      data.color = defaultColors[n % defaultColors.length];
                    }

                    // 비트 메세지 파싱
                    if (twitchArgs.bits) {
                      data.cheers = Number(twitchArgs.bits);
                    }
      ...
      { "./cheer_list.json": 1, "./twip_apply.js": 3 },
    ],
    3: [
      function (require, module, exports) {
        var twipMsg =
